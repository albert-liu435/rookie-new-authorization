...
const getCaptcha = () => {
  axios({
    method: 'GET',
    // url: 'http://192.168.2.211:8080/getCaptcha'
        url: 'http://rookie-authorization.server.com:8080/getCaptcha'
  }).then((r) => {
    let result = r.data
    if (result.success) {
...
const submitLogin = () => {
  ...
  axios({
    method: 'post',
    // url: 'http://192.168.2.211:8080/login',
    url: 'http://rookie-authorization.server.com:8080/login',
    headers: {
...
const getSmsCaptcha = () => {
  ...
  axios({
    method: 'get',
    url: `http://rookie-authorization.server.com:8080/getSmsCaptcha?phone=${loginModel.value.username}`
  }).then((r) => {
    let result = r.data
    if (result.success) {
